...
# ---- migrated from tests/integration/test_skip_dependency_service_branches.py ----
...
@pytest.mark.asyncio
async def test_get_transitive_filters_inactive_task_before_eligibility() -> None:
    """Paused downstream tasks are filtered before eligibility checks."""
    from types import SimpleNamespace
    ...
    paused_task = SimpleNamespace(id="down", record_state=PAUSED)
    tasks_mr = MagicMock()
    tasks_mr.scalars.return_value.all.return_value = [paused_task]
    ...
    with patch.object(
        ...
        out = await get_transitive_hard_dependents_toposort(
            mock_db, "start", "u1", None, None
        )
