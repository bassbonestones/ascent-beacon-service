...
def test_recurring_no_scheduled_at_utc_timezone() -> None:
    ...
    assert out == datetime(2030, 5, 5, 23, 59, 59, 999999, tzinfo=timezone.utc)
...
def test_one_shot_scheduled_date_with_tz() -> None:
    ...
    assert getattr(out.tzinfo, "key", None) == "Europe/London"
...
def test_one_shot_no_schedule_end_of_day() -> None:
    ...
    assert out == datetime(2030, 3, 3, 23, 59, 59, 999999, tzinfo=timezone.utc)
...
def test_recurring_scheduled_at_invalid_tz_falls_back_to_template_tz() -> None:
    ...
    assert out == datetime(2030, 4, 4, 14, 0, 0, tzinfo=timezone.utc)
