...
class TestCheckHardDependents:
    ...
    async def test_returns_downstream_tasks(self) -> None:
        ...
        mock_rule.downstream_task.id = "task-2"
        mock_rule.downstream_task.title = "Downstream Task"
        mock_rule.downstream_task.is_recurring = True

        mock_result = MagicMock()
        mock_result.scalars.return_value.all.return_value = [mock_rule]
        mock_db.execute.return_value = mock_result

        affected = await check_hard_dependents(mock_db, "task-1", "user-1")

        assert len(affected) == 1
        assert affected[0]["task_id"] == "task-2"
        assert affected[0]["task_title"] == "Downstream Task"
