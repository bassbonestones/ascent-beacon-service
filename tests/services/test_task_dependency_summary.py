...
@pytest.mark.asyncio
async def test_build_task_dependency_summaries_for_day_no_anchors(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """When no anchors exist, service should return single default slot."""
    from app.schemas.tasks import TaskDependencySummary

    async def _fake_summary_for_anchor(*args, **kwargs):
    ...
    monkeypatch.setattr(tds, "list_dependency_anchors_for_day", lambda *a, **k: [])
    monkeypatch.setattr(tds, "_summary_for_anchor", _fake_summary_for_anchor)
    ...
    out = await tds.build_task_dependency_summaries_for_day(
        AsyncMock(), "user-1", _Task(), "2026-06-10", None
    )
    assert list(out.keys()) == [""]


@pytest.mark.asyncio
async def test_build_task_dependency_summaries_for_day_marks_later_slots_blocked(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Sequential hard deps should mark slots after first pending as blocked."""
    from app.schemas.tasks import TaskDependencySummary

    anchors = [
        ("occ1", datetime(2026, 6, 10, 9, 0, tzinfo=timezone.utc)),
        ("occ2", datetime(2026, 6, 10, 13, 0, tzinfo=timezone.utc)),
        ("occ3", datetime(2026, 6, 10, 17, 0, tzinfo=timezone.utc)),
    ]
    ...
    async def _rows_async(*args, **kwargs):
        return []

    monkeypatch.setattr(tds, "list_dependency_anchors_for_day", lambda *a, **k: anchors)
    monkeypatch.setattr(tds, "_summary_for_anchor", _fake_summary_for_anchor)
    monkeypatch.setattr(tds, "downstream_has_sequential_slot_hard_dependency", _true_async)
    monkeypatch.setattr(tds, "completions_for_task_local_date", _rows_async)
    monkeypatch.setattr(tds, "first_pending_slot_index", lambda *a, **k: 0)
    ...
    assert out["occ1"].has_unmet_hard is False
    assert out["occ2"].has_unmet_hard is True
    assert out["occ3"].has_unmet_hard is True
    assert out["occ2"].readiness_state == "blocked"


def test_first_slot_summary_empty_returns_none() -> None:
    assert tds.first_slot_summary({}) is None
